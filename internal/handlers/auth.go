...
func Login(c *fiber.Ctx) error {
	...
	if email == nil && user == nil {
		return c.Status(fiber.StatusUnauthorized).JSON(responses.Error("error", "User not found", err))
	}
	...
	if !CheckPasswordHash(pass, ud.Password) {
		return c.Status(fiber.StatusUnauthorized).JSON(responses.Error("error", "Invalid password", err))
	}
	...
	claims["expired_at"] = time.Now().Add(time.Minute * 15).Unix()
